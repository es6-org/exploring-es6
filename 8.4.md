...
- 模板对象（The template object）：这里你会得到一组转义后模板字符串，它的数量永远都是替换表达式的数量加1，如果替换表达式在模板字面量的开头，第一个模板字符串为空字符串，如果替换表达式在模板字面量的末尾，最后一个模板字符串为空字符串。
- 替换表达式（The substitutions）：它的值会变成后面的参数。

想法是，同样的模板字面量可能被执行多次（例如，在一个循环或函数中）。With the template object, 标签函数可以从先前调用中缓存数据：（1）可能是缓存的数据，（2）每次调用的改变。Caching happens per realm (think frame in a browser). That is, there is one template object per call site and realm.


> Tagged templates in the spec

> 在标签化模板的部分解释了它们是如何演示函数调用。另一个单独的部分解释模板字面量是如何变成一个参数列表：模板对象和替换表达式。

### 8.4.1 Escaping in tagged templates: cooked vs. raw

在标签化模板中，有更多转义规则，因为模板字符串(引号内的文本片段，不包括替换表达式)在未转义（cooked）和转义（raw）这两种解释下都是可用的。规则如下:

在未转义（cooked）和转义（raw）两种解释中，在美元符号（`$`）前的反斜线（`\`）防止 `${` 被解释为一个替换表达式的开始。然而，每一个反斜杠在转义（raw）的解释中都要被提到，甚至是那些转义的替换表达式。标签函数 describe 让我们去探索这意味着什么。
...
在转义（raw）模式下反斜杠唯一有效果的是当它出现在替换表达式前面（会转义替换表达）。
...
#### 8.4.2.1 Defining and using an HTML template
...
每个替换表达式总是被模板字符串包围。如果标签化模板以替换表达式结束，最后一个模板字符串是一个空字符串。因此，下面的表达式总是真:
...
#### 8.4.2.4 Should I use this in production code?
...
### 8.4.3 Example: quoting parts of regular expressions
